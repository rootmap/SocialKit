...
$sqlquery = $obj->FlyQuery("SELECT 
                                ...
                                WHERE b.group_id='" . $new_group_id . "' AND b.id='".$_GET['view']."' 
                                GROUP BY b.id
...
if (!empty($sqlquery)) {
    foreach ($sqlquery as $post):
        ...
        if ($post->post_status == "1") {
            ...
            $status_published = 'in publically.';
            if ($post->user_id == $input_by) {
                $link .='<li role="presentation"><a role="menuitem" tabindex="-1" class="dostums-post-edit" id="' . $post->id . '" href="group_view.php?view=' . $post->id . '&group_id=' . $post->group_id . '"><i class="fa fa-list"></i> View Post </a></li>
        ...
        <div class="panel panel-default  panel-customs-post">
            ...
            <div class="panel-bottom">
                <div class="panel-footer has-share-panel">
                    <div class="row">
                        <div class="col-sm-12">
                            <?php include('status/postactionbar_group.php'); ?>
                        </div>
                    </div>
                </div>
