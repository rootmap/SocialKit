<div class="share-panel">
    <button type="button" id="<?php echo $post->id; ?>" class="sp-like def_button"><i class="fa fa-thumbs-up"></i> <span id="postlike<?php echo $post->id; ?>"> <?php
            if ($post->likes != 0) {
                echo $post->likes;
                ?>
                likes <?php } else { ?> Like <?php } ?></span></button>
    ...
    ?>
    <button type="button" id="<?php echo $post->id; ?>" class="sp-share def_button"><i class="fa fa-share"></i> <?php echo $sharecount; ?> shares</button>
...
<?php } ?> 
    ...
    <?php 
    if(($post->user_id == $input_by) || in_array($input_by,$admin_list))
    {
    ?>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#statusPermission_<?php echo $post->id; ?>').change(function () {
                ...
                if (statusPermission != '')
                {
                    $.post('./lib/status.php', {'st': 44, 'user_id': userid, 'post_id': post_id, 'group_id': group_id, 'stp': statusPermission}, function (data) {
                        if (data == 1)
                        {
                            var prht=$('#statusPermission_<?php echo $post->id; ?> option[value='+statusPermission+']').html();
                            $('#permissionstatuslabel_<?php echo $post->id; ?>').text(prht);
                            //  alert(prht);
                            $('#static_permission_<?php echo $post->id; ?>').toggle();
                            $('#dynamicpermission_<?php echo $post->id; ?>').toggle();
                        }
