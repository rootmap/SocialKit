"use strict";
var FS = require("fs"), PATH = require("path"), UTIL = require("util"), CRYPTO = require("crypto"), SLASH = PATH.normalize("/"), NULL_PATH_ERROR = new Error("Null path."), SECURITY_READ_ERROR = new Error("Security error, cannot read."), SECURITY_WRITE_ERROR = new Error("Security error, cannot write."), SECURITY_READ_WRITE_ERROR = new Error("Security error, cannot read nor write."), SM = null, updateFileProperties = function (a, b) {
    a._path = null, a._usablePath = null, a._isAbsolute = !1;
    if (b === undefined || b === null)return;
    b = PATH.normalize(b);
    var c = b.indexOf(":") + 1, d = b.substring(0, c);
    ...
    var b = process.mainModule.filename, c = b.substring(0, b.lastIndexOf(SLASH)), d = PATH.relative(process.cwd(), c), e = this;
    ...
    FS.stat(a, function (e, f) {
        if (e)d && d(e, !1); else {
            var g = (f.mode & parseInt("777", 8)).toString(8), h = parseInt(g[0]), i = !!(h & b);
            if (i && !c || !i && c) {
                var j = c ? b : -b;
...
File.prototype.canExecute = function (a) {
    ...
    FS.stat(this._usablePath, function (e, f) {
        if (e)c(e, null); else if (f.isDirectory())c("The abstract path is a directory.", null); else if (f.isFile()) {
    ...
    var f = Math.floor(Math.random() * 1e12), g = new File(PATH.join(e, c + f + d));
    PATH.exists(g._usablePath, function (c) {
        if (c)File.createTempFile(a, b); else {
            g.removeOnExit();
    ...
    FS.stat(this._usablePath, function (c, e) {
        if (c)b(c, null); else if (e.isFile())b("The path is not a directory.", null); else if (e.isDirectory()) {
...
var removeSynchronous = function (a) {
    ...
    if (b.isFile())FS.unlinkSync(a._usablePath); else if (b.isDirectory()) {
...
SecurityManager.NONE = Object.freeze({id: 0}), SecurityManager.READ = Object.freeze({id: 1}), SecurityManager.WRITE = Object.freeze({id: 2}), SecurityManager.READ_WRITE = Object.freeze({id: 3});
var checkSecurity = function (a) {
    ...
    var b = function (a) {
        ...
        for (var f in c) {
            var g = c[f], h = g.directory.replace(/\\/g, "/"), i = new RegExp("^" + h);
...
SecurityManager.prototype.allow = function (a, b) {
    this._allow.push({directory: getAbsolutePath(a), permissions: b ? b : SecurityManager.READ_WRITE})
}, SecurityManager.prototype.deny = function (a, b) {
    this._deny.push({directory: getAbsolutePath(a), permissions: b ? b : SecurityManager.READ_WRITE})
