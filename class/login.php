	...
	function login_user($username, $password){
        ...
        if (empty($username) or empty($password)) {
			...
			if($chkauth!=1)
			{
				...
				{
					if($obj->exists_multiple($table,array("username"=>$username,"password"=>$this->MakePassword($password),"status"=>1,"user_permission"=>1)) == 1) {
						...
						session_write_close();
						return $plugin->Success("Thank You For Login As A Member,Welcome To Your Account.", $success);
					} 
					else 
					{
						if($obj->exists_multiple($table,array("username"=>$username,"password"=>$this->MakePassword($password),"status"=>1,"user_permission"=>2)) == 1){
							return $plugin->Error("Your Account is Suspended. Please Contact With Systech Unimax Ltd.", $obj->filename());
							}
						elseif($obj->exists_multiple($table,array("username"=>$username,"password"=>$this->MakePassword($password),"status"=>1,"user_permission"=>3)) == 1){
							return $plugin->Error("Your Account is Blocked as Spam. Please Contact With Systech Unimax Ltd.", $obj->filename());
							}
			...
			{
				return $plugin->Error("Warning, Your network pc is not authorized.",$obj->filename());
			}
        }
    }
